...
// --- 2. THE 26+ COMMAND LIST (NO MODERATION) ---
const commands = [
    // FUN
...
client.once('ready', async () => {
    ...
    try {
        ...
        console.log("🚀 Mb's Ai: Everything is LIVE!");
...
async function getAIResponse(msg) {
    try {
        ...
        return res.data.message || "My brain didn't catch that";
    } catch { return "My brain didn't catch that"; }
}
    ...
    switch (i.commandName) {
        case 'help':
            const h = new EmbedBuilder().setTitle("🤖 Mb's Ai - Super Menu").addFields(
                ...
                { name: "🛠️ Utility", value: "`whois`, `avatar`, `ping`, `roles`, `stats`, `serverinfo`, `setbirthday`, `calculate`, `suggest`" }
            ).setColor(BOT_COLOR);
        ...
        case '8ball':
            const responses = ["Yes", "No", "Maybe", "Most likely", "Don't count on it"];
            return i.editReply(`🎱 **Answer:** ${responses[Math.floor(Math.random()*responses.length)]}`);
